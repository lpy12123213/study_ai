    ...
    <header className="flex h-14 items-center justify-between gap-3 border-b bg-background px-4">
      <div className="flex min-w-0 items-center gap-2">
        ...
        <Input
          ...
          className="h-9 w-56 border-0 bg-transparent px-2 text-base font-semibold shadow-none focus-visible:ring-1"
        />
        <Input
          value={props.subject}
      ...
      <div className="flex items-center gap-2">
        <Button type="button" variant="outline" onClick={props.onAddNote}>
          <Plus className="h-4 w-4" />
          便签
        </Button>
        <Button type="button" variant="outline" onClick={props.onOpenPick}>
          <Search className="h-4 w-4" />
          选题
        </Button>
        <Button type="button" variant="outline" onClick={props.onOpenVersions}>
          <History className="h-4 w-4" />
          版本
        </Button>
        <Button type="button" onClick={props.onSave} disabled={props.saving || !props.dirty}>
