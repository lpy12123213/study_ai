...
import type { QuestionLibraryFilters } from '@/features/questionLibrary/hooks/useQuestionLibrary'
  ...
  const runScore = useCallback(
    (payload: Omit<ScoreQuestionLibraryBatchPayload, 'task_id'> & { task_id?: string }) => {
      ...
      apiClient
        ...
        .catch((err: any) => {
          const message = typeof err?.message === 'string' ? err.message : 'score_failed'
