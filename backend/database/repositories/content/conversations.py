from __future__ import annotations

import json
from typing import List, Optional

from sqlalchemy import delete, func, select
from sqlalchemy.ext.asyncio import AsyncSession

from backend.core.time_utils import utcnow_naive
from backend.database.engine import async_session_maker
from backend.database.schema import Conversation, Message


def _normalize_user_id(user_id: str) -> str:
    return str(user_id or "").strip()[:64]


def _require_user_id(user_id: str) -> str:
    uid = _normalize_user_id(user_id)
    if not uid:
        raise ValueError("missing_user_id")
    return uid


async def create_conversation(
    *,
    user_id: str,
    title: str = "新对话",
    session: Optional[AsyncSession] = None,
) -> int:
    uid = _require_user_id(user_id)

    own = session is None
    if own:
        async with async_session_maker() as session:
            conv_id = await create_conversation(user_id=uid, title=title, session=session)
            await session.commit()
            return conv_id

    conv = Conversation(user_id=uid, title=str(title or "").strip() or "新对话")
    session.add(conv)
    await session.flush()
    await session.refresh(conv)
    return int(conv.id)


async def list_conversations(
    *,
    user_id: str,
    limit: int = 50,
    session: Optional[AsyncSession] = None,
) -> List[dict]:
    uid = _require_user_id(user_id)
    own = session is None
    if own:
        async with async_session_maker() as session:
            return await list_conversations(user_id=uid, limit=limit, session=session)

    stmt = (
        select(Conversation)
        .where(Conversation.user_id == uid)
        .order_by(Conversation.updated_at.desc())
        .limit(int(limit or 50))
    )
    result = await session.execute(stmt)
    convs = result.scalars().all()
    return [
        {
            "id": c.id,
            "user_id": c.user_id,
            "title": c.title,
            "created_at": c.created_at.isoformat() if c.created_at else "",
            "updated_at": c.updated_at.isoformat() if c.updated_at else "",
        }
        for c in convs
    ]


async def get_conversation(
    *,
    user_id: str,
    conv_id: int,
    session: Optional[AsyncSession] = None,
) -> Optional[dict]:
    uid = _require_user_id(user_id)
    own = session is None
    if own:
        async with async_session_maker() as session:
            return await get_conversation(user_id=uid, conv_id=conv_id, session=session)

    result = await session.execute(
        select(Conversation).where(
            Conversation.id == int(conv_id),
            Conversation.user_id == uid,
        )
    )
    conv = result.scalar_one_or_none()
    if not conv:
        return None
    return {
        "id": conv.id,
        "user_id": conv.user_id,
        "title": conv.title,
        "created_at": conv.created_at.isoformat() if conv.created_at else "",
        "updated_at": conv.updated_at.isoformat() if conv.updated_at else "",
    }


async def update_conversation_title(
    *,
    user_id: str,
    conv_id: int,
    title: str,
    session: Optional[AsyncSession] = None,
) -> bool:
    uid = _require_user_id(user_id)
    own = session is None
    if own:
        async with async_session_maker() as session:
            ok = await update_conversation_title(user_id=uid, conv_id=conv_id, title=title, session=session)
            await session.commit()
            return ok

    result = await session.execute(
        select(Conversation).where(
            Conversation.id == int(conv_id),
            Conversation.user_id == uid,
        )
    )
    conv = result.scalar_one_or_none()
    if not conv:
        return False
    conv.title = str(title or "").strip() or "新对话"
    conv.updated_at = utcnow_naive()
    await session.flush()
    return True


async def delete_conversation(
    *,
    user_id: str,
    conv_id: int,
    session: Optional[AsyncSession] = None,
) -> bool:
    uid = _require_user_id(user_id)
    own = session is None
    if own:
        async with async_session_maker() as session:
            ok = await delete_conversation(user_id=uid, conv_id=conv_id, session=session)
            await session.commit()
            return ok

    result = await session.execute(
        select(Conversation).where(
            Conversation.id == int(conv_id),
            Conversation.user_id == uid,
        )
    )
    conv = result.scalar_one_or_none()
    if not conv:
        return False
    await session.delete(conv)
    await session.flush()
    return True


async def delete_all_conversations(
    *,
    user_id: str,
    session: Optional[AsyncSession] = None,
) -> dict:
    """Delete all chat conversations and their messages.

    Notes:
    - Only clears `conversations` + `messages` for the given user.
    - Does not touch papers/canvas/etc.
    """

    uid = _require_user_id(user_id)
    own = session is None
    if own:
        async with async_session_maker() as session:
            payload = await delete_all_conversations(user_id=uid, session=session)
            await session.commit()
            return payload

    conv_ids_result = await session.execute(select(Conversation.id).where(Conversation.user_id == uid))
    conv_ids = [int(x) for x in conv_ids_result.scalars().all()]
    if not conv_ids:
        return {"conversations": 0, "messages": 0}

    conv_count = len(conv_ids)
    msg_result = await session.execute(select(func.count(Message.id)).where(Message.conversation_id.in_(conv_ids)))
    msg_count = int(msg_result.scalar() or 0)

    await session.execute(delete(Message).where(Message.conversation_id.in_(conv_ids)))
    await session.execute(delete(Conversation).where(Conversation.user_id == uid))
    await session.flush()
    return {"conversations": conv_count, "messages": msg_count}


async def add_message(
    *,
    user_id: str,
    conv_id: int,
    role: str,
    content: str,
    tool_calls: Optional[str] = None,
    tool_call_id: Optional[str] = None,
    session: Optional[AsyncSession] = None,
) -> int:
    uid = _require_user_id(user_id)
    own = session is None
    if own:
        async with async_session_maker() as session:
            msg_id = await add_message(
                user_id=uid,
                conv_id=conv_id,
                role=role,
                content=content,
                tool_calls=tool_calls,
                tool_call_id=tool_call_id,
                session=session,
            )
            await session.commit()
            return msg_id

    result = await session.execute(
        select(Conversation).where(
            Conversation.id == int(conv_id),
            Conversation.user_id == uid,
        )
    )
    conv = result.scalar_one_or_none()
    if not conv:
        raise ValueError("conversation_not_found")
    conv.updated_at = utcnow_naive()

    msg = Message(
        conversation_id=int(conv_id),
        role=str(role or "").strip(),
        content=str(content or ""),
        tool_calls=tool_calls,
        tool_call_id=tool_call_id,
    )
    session.add(msg)
    await session.flush()
    await session.refresh(msg)
    return int(msg.id)


async def get_messages(
    *,
    user_id: str,
    conv_id: int,
    limit: int = 200,
    before_id: Optional[int] = None,
    include_trace: bool = False,
    include_tool_content: bool = False,
    tool_content_max_chars: int = 2000,
    session: Optional[AsyncSession] = None,
) -> List[dict]:
    uid = _require_user_id(user_id)
    limit = max(1, min(int(limit or 200), 100))
    tool_content_max_chars = max(0, min(int(tool_content_max_chars or 2000), 200_000))

    before_id_value: Optional[int] = None
    if before_id is not None:
        try:
            before_id_value = int(before_id)
        except (TypeError, ValueError):
            before_id_value = None

    own = session is None
    if own:
        async with async_session_maker() as session:
            return await get_messages(
                user_id=uid,
                conv_id=conv_id,
                limit=limit,
                before_id=before_id,
                include_trace=include_trace,
                include_tool_content=include_tool_content,
                tool_content_max_chars=tool_content_max_chars,
                session=session,
            )

    # Join conversations to enforce user scoping without an extra round-trip.
    stmt = (
        select(Message)
        .join(Conversation, Message.conversation_id == Conversation.id)
        .where(
            Conversation.id == int(conv_id),
            Conversation.user_id == uid,
        )
    )
    if before_id_value and before_id_value > 0:
        stmt = stmt.where(Message.id < int(before_id_value))
    if not include_trace:
        # Default view: user + final assistant messages only.
        stmt = stmt.where(Message.role != "tool")
        stmt = stmt.where(Message.tool_calls.is_(None))

    # Fetch latest N then reverse to chronological order for the frontend.
    stmt = stmt.order_by(Message.id.desc()).limit(limit)
    result = await session.execute(stmt)
    msgs = list(result.scalars().all())
    msgs.reverse()

    out: List[dict] = []
    for m in msgs:
        tool_calls = None
        if m.tool_calls:
            try:
                tool_calls = json.loads(m.tool_calls)
            except (json.JSONDecodeError, TypeError):
                tool_calls = None

        content = m.content or ""
        tool_result_meta = None
        if m.role == "tool":
            tool_result_meta = {"size": len(content), "success": None, "error": None}
            try:
                payload = json.loads(content) if content else None
                if isinstance(payload, dict):
                    if isinstance(payload.get("success"), bool):
                        tool_result_meta["success"] = payload.get("success")
                    err = payload.get("error")
                    if isinstance(err, str) and err.strip():
                        tool_result_meta["error"] = err.strip()[:500]
            except (json.JSONDecodeError, TypeError):
                tool_result_meta["error"] = tool_result_meta.get("error") or "invalid_tool_payload"

            if not include_tool_content:
                content = ""
            elif tool_content_max_chars > 0 and len(content) > tool_content_max_chars:
                content = content[:tool_content_max_chars] + "...(truncated)"

        out.append(
            {
                "id": m.id,
                "role": m.role,
                "content": content,
                "tool_calls": tool_calls,
                "tool_call_id": m.tool_call_id,
                "created_at": m.created_at.isoformat() if m.created_at else "",
                "tool_result_meta": tool_result_meta,
            }
        )
    return out


async def fork_conversation(
    *,
    user_id: str,
    parent_conv_id: int,
    until_message_id: int,
    title: Optional[str] = None,
    session: Optional[AsyncSession] = None,
) -> dict:
    """Fork a conversation by copying messages up to `until_message_id` (inclusive)."""

    uid = _require_user_id(user_id)
    parent_conv_id = int(parent_conv_id or 0)
    until_message_id = int(until_message_id or 0)
    if parent_conv_id <= 0 or until_message_id <= 0:
        raise ValueError("invalid_parent_or_message_id")

    own = session is None
    if own:
        async with async_session_maker() as session:
            payload = await fork_conversation(
                user_id=uid,
                parent_conv_id=parent_conv_id,
                until_message_id=until_message_id,
                title=title,
                session=session,
            )
            await session.commit()
            return payload

    parent_result = await session.execute(
        select(Conversation).where(
            Conversation.id == parent_conv_id,
            Conversation.user_id == uid,
        )
    )
    parent = parent_result.scalar_one_or_none()
    if not parent:
        raise ValueError("conversation_not_found")

    msgs_result = await session.execute(
        select(Message).where(Message.conversation_id == parent_conv_id).order_by(Message.created_at)
    )
    msgs = list(msgs_result.scalars().all())
    if not msgs:
        raise ValueError("conversation_empty")

    copied: List[Message] = []
    copied_visible_count = 0
    found = False
    for m in msgs:
        copied.append(m)
        if m.role != "tool":
            copied_visible_count += 1
        if m.id == until_message_id:
            found = True
            break
    if not found:
        raise ValueError("message_not_found")

    new_title = str(title or "").strip() or f"{parent.title} - 分支"
    conv = Conversation(user_id=uid, title=new_title)
    session.add(conv)
    await session.flush()

    for m in copied:
        session.add(
            Message(
                conversation_id=int(conv.id),
                role=m.role,
                content=m.content,
                tool_calls=m.tool_calls,
                tool_call_id=m.tool_call_id,
            )
        )

    conv.updated_at = utcnow_naive()
    await session.flush()

    return {
        "id": conv.id,
        "title": conv.title,
        "parent_conversation_id": parent_conv_id,
        "forked_from_message_id": until_message_id,
        "copied_message_count": len(copied),
        "copied_visible_message_count": copied_visible_count,
    }
