...
from backend.integrations.mcp.search.http_client import get_mcp_search_http_client
...
_DEFAULT_HTTP_CLIENT_FACTORY = get_mcp_search_http_client


async def _post_search(client: Any, *, headers: dict[str, str], payload: dict[str, Any]) -> Any:
    try:
        return await client.post(
            f"{TAVILY_BASE_URL}/search",
            headers=headers,
            json=payload,
            timeout=float(TAVILY_TIMEOUT or 60),
        )
    except TypeError:
        return await client.post(
            f"{TAVILY_BASE_URL}/search",
...
async def tavily_search(
    ...
    try:
        if get_mcp_search_http_client is _DEFAULT_HTTP_CLIENT_FACTORY:
            async with httpx.AsyncClient(timeout=float(TAVILY_TIMEOUT or 60)) as client:
                response = await _post_search(client, headers=headers, payload=payload)
        else:
            client = await get_mcp_search_http_client()
            response = await _post_search(client, headers=headers, payload=payload)
...
from backend.integrations.mcp.search.registry import PROVIDERS  # noqa: E402
