...
async def _search_repositories_get(
    client: Any,
    *,
    url: str,
    params: dict[str, Any],
...
async def github_search_repositories(
    ...
    *,
    ...
    for attempt in range(3):
        try:
            request_timeout = min(max(timeout, 10.0), 60.0)
            request_headers = _build_headers(token=effective_token)
            ...
            if get_mcp_search_http_client is _DEFAULT_HTTP_CLIENT_FACTORY:
                async with httpx.AsyncClient(
                    ...
                    resp = await _search_repositories_get(
                        client,
                        url=url,
                        params=params,
                        headers=request_headers,
            ...
            if status_code in {401, 403}:
                ...
                msg = f"GitHub API rejected request (status={status_code})."
                try:
                    data_err = resp.json()
