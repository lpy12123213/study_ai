"""MCP stdio tool definitions, grouped by domain.

Split out of ``stdio_tools.py`` (which had grown into a single 1300-line
function): each builder returns one domain slice of the stdio Tool list, and
``stdio_tools.get_stdio_tools()`` concatenates them in the original order.
"""
...
def question_bank_tools() -> List[Tool]:
    """题库/组卷网检索与题目管理工具"""
    return [
        Tool(
...
def paper_compose_tools() -> List[Tool]:
    """蓝图组卷工具"""
    return [
        Tool(
...
def agent_knowledge_tools() -> List[Tool]:
    """知识检索/讲解/用户画像等 Agent 工具"""
    return [
        Tool(
...
def search_and_compute_tools() -> List[Tool]:
    """上下文压缩、联网搜索与科学计算工具"""
    return [
        Tool(
...
def paper_analysis_tools() -> List[Tool]:
    """试卷求解/课标对齐/试卷对比工具"""
    return [
        Tool(
            name="solve_paper",
