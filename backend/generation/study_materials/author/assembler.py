...
and rejects machine fallback notes before they leak into the final prose (the
legacy ``study_archive.py`` defect where "未成功使用模型生成" notes reached readers).
...
def assemble(
    ...
    a document without figures or references, is legal.
    """
    ...
    if references:
        ...
        for idx, ref in enumerate(references, start=1):
            lines.append(f"[{idx}] {ref.get('title', '')} — {ref.get('url', '')}")
        out = out.rstrip() + "\n\n" + "\n".join(lines) + "\n"
