    ...
    *,
    ...
    if t in {"paper_compose", "paper_generate_full"}:
        ...
        return AgentRunSpec(
            ...
            budget=AgentBudget(
                max_llm_calls=28 if is_full else 16,
                max_tool_calls=60 if is_full else 40,
                max_iterations=24 if is_full else 16,
                max_runtime_s=1500 if is_full else 900,
            ),
            output_contract={"kind": "paper", "formats": ["paper", "tex", "pdf"], "requires_confirmation": not is_full},
            resume_state=resume,
            metadata=_metadata(t, weight="heavy" if is_full else "medium", extra={"sandbox": "docker-optional"}),
        )
