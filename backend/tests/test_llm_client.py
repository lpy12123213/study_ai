...
class RespErrorTests(unittest.TestCase):
    ...
    def test_decode_chat_response_payload_detects_html_body(self) -> None:
        resp = httpx.Response(
            200,
        ...
        data, error = llm_client._decode_chat_response_payload(resp)

        self.assertIsNone(data)
        self.assertEqual(error, "html_response_body")
...
class ChatCompletionReasoningCompatTests(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_non_stream_sse_body_is_assembled_into_content(self) -> None:
        req = httpx.Request("POST", "https://example.test/chat/completions")
        responses = [
            httpx.Response(
                200,
                text=(
                    ...
                    'data: {"choices":[{"delta":{"content":"world"},"finish_reason":"stop"}],"usage":{"total_tokens":2}}\n'
                    "data: [DONE]\n"
                ),
        ...
        self.assertEqual(res.usage.get("total_tokens"), 2)

    async def test_custom_provider_fallback_retries(self) -> None:
