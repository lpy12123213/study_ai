...
def _heuristic_type(kp: str) -> str:
    ...
    if not s:
        return "concept"
    if any(x in s for x in ["定理", "命题", "引理", "推论", "结论"]):
        return "theorem"
    if any(x in s for x in ["算法", "排序", "搜索", "动态规划", "DP", "贪心", "回溯"]):
        return "algorithm"
    if any(x in s for x in ["实验", "测量", "装置", "观测", "现象"]):
        return "experiment"
    if any(x in s for x in ["历史", "发展", "人物", "年代", "起源"]):
        return "history"
    if any(x in s for x in ["定义", "是什么", "含义", "概念"]):
        return "definition"
    return "concept"
