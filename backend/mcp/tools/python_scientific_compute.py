...
_BANNED_CALL_NAMES = {"eval", "exec", "compile", "open", "__import__", "input", "help", "globals", "locals", "vars", "dir", "getattr", "setattr", "delattr"}
...
def _run_code_isolated(code: str, timeout_seconds: int) -> Dict[str, Any]:
    # File lives under `backend/mcp/tools/`, so repo root is 3 levels up.
    repo_root = Path(__file__).resolve().parents[3]
    wrapper = (
        "import json,sys;"
        "from backend.mcp.tools.python_scientific_compute import _run_code;"
        "src=sys.stdin.read();"
        "res=_run_code(src);"
