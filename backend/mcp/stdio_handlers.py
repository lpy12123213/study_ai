...
async def handle_tool_call(server: Any, name: str, arguments: Any) -> Sequence[TextContent]:
    ...
    try:
        if name == "search_questions_by_keyword":
            ...
            if resolved_subject != server.current_subject:
                server.current_subject = resolved_subject
                server.crawler.set_subject(resolved_subject)
            ...
            if export_subject:
                ...
                if server.crawler is not None:
                    server.crawler.set_subject(export_subject)
            ...
            if target_subject:
                ...
                server.crawler.set_subject(target_subject)
            ...
            # 切换学科
            server.current_subject = subject
            if server.crawler is not None:
                server.crawler.set_subject(subject)
