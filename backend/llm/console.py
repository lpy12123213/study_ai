...
_RAW_STREAM_NEEDS_NEWLINE: Dict[str, bool] = {}
...
def meta_enabled() -> bool:
    raw = os.getenv("LLM_CONSOLE_META")
    if raw is None:
        raw = "0"
    return _truthy(raw)
...
def log_delta(*, req_id: str, channel: str, text: str) -> None:
    ...
    if channel == "content":
        _safe_write(t)
        _RAW_STREAM_NEEDS_NEWLINE[req_id] = not t.endswith(("\n", "\r"))
        return
    if not meta_enabled():
        return
...
def log_end(
    *,
    ...
    if not enabled():
        return
    needs_newline = _RAW_STREAM_NEEDS_NEWLINE.pop(req_id, False)
    if needs_newline:
        _safe_write("\n")
    if not meta_enabled():
        if error:
            _safe_print(f"[llm:{req_id}] end error={error}")
        return
    parts = []
    if elapsed_s > 0:
