...
def create_default_prompt_registry() -> PromptRegistry:
    ...
    json_specs = [
        ...
        (
            "study.author.split.v1",
            "你是严谨的自学教材结构设计助手。根据输入的主题、学科、preset、知识点数量范围"
            " min_points~max_points 与全局输出要求，"
            "把主题拆分为适合自学的知识点列表。\n"
            "输出 JSON 对象，字段：\n"
            "- knowledge_points：知识点名称列表（字符串数组）。\n"
            "要求：\n"
            "- 数量 min_points~max_points 个，不得超出范围。\n"
            "- 按学习顺序排列：先修概念在前，综合应用在后。\n"
            "- 每个知识点是独立可教学的概念点，粒度适中（一节课能讲透）。\n"
            "- 知识点之间不重叠、不互相包含；不得把多个概念合并为一项，也不得把同一概念拆成多项。\n"
            "- 同一公式、同一机制或同一操作的参数取值、步骤与算例必须合并到一个分类/流程知识点，"
            "不得分别占用多个名额。\n"
            "- 名额有限时优先覆盖定义与意义、成立条件、核心机制、边界与反例、易混概念辨析、综合应用。\n"
            "- 知识点名称简洁具体（2-15 字），与主题同一语言。\n"
            "- 输入若列出必须逐字包含的受控拓展主题，须各自保留为独立知识点，放在高中主线之后。\n"
            "Output fields: knowledge_points.",
        ),
        (
            ...
            "前置说明与总结由全书骨架负责，不得创建不对应知识点的额外 section。\n"
              "- 必须把主题与附加要求中的每一项明确任务逐项分配到最相关的 sections[].key_points；"
              "时间顺序、因果中间机制、误区纠正不得遗漏；辨析/对比任务必须列出要比较双方。\n"
              "- deep/research 资料须主动安排至少4组与主题直接相关的易混概念辨析，"
              "每组在 key_points 同时点名双方并要求写出明确比较语义；研究笔记有足够出处时，"
              "还须分散安排至少4个互不重复的真实误区及明确纠正。\n"
              "- 受控拓展 section 的 title 必须使用精确的 [拓展:主题全名] 标记；"
            "key_points 必须要求正文给出 [高中连接]，说明其对高中学习的具体帮助。\n"
        ...
        (
            "study.author.learning_repair.v1",
        ...
        (
            "study.author.figure_table_fallback.v1",
            "你是严谨的自学教材编辑。一张示意图渲染失败，请把该图要传达的信息改写为一个 Markdown 表格，"
            "作为学习者可见的降级替代。\n"
            "要求：\n"
            "- 只输出一个表格：表头行、分隔行与至少两行数据。\n"
            "- 内容基于给定的配图意图、小节要点与图注，不得引入新事实。\n"
            "- 禁止输出代码围栏、URL、脚注、[[...]] 占位符或表格之外的说明文字。\n"
            "Output fields: markdown table only.",
        ),
        (
    ...
    markdown_specs = [
        ...
        (
            ...
            True,
            "你是严谨的自学教材作者。根据给定蓝图撰写全书骨架，输出 Markdown，整体结构必须依次为：\n"
            "1. 首行书名，形如 # 自学材料：<主题>；\n"
            "2. 紧随其后的 meta 引用行（> 开头的引用块），注明学科与生成预设；\n"
            "3. ## 使用方式（建议） 小节：2-4 条阅读与自测建议；\n"
            "4. ## 知识点目录 小节：逐项列出全部正文小节，每项必须是可跳转锚点链接，"
            "形如 - [一、标题](#一标题)；\n"
            "5. 全书导言；\n"
            "6. 正文小节：每节用编号标题 ## N、标题（标题与蓝图 section title 保持一致），"
            "节首写引入段，正文位置只留占位符 [[FILL:sec-id]]（sec-id 为蓝图中的 section id），"
            "图位留占位符 [[FIG:n]]；节间写衔接段；\n"
            "7. 全书总结章；\n"
            "8. 术语表。\n"
            "占位符纪律：\n"
            "- 蓝图中每个 section 必须恰好对应一个 [[FILL:sec-id]] 占位符，不得多也不得少。\n"
            "- 每个 [[FILL:sec-id]] 与 [[FIG:n]] 占位符必须单独占一行，不与正文同段。\n"
            "- 占位符前后禁止出现任何「占位」字样；禁止添加「正文占位」之类仅为占位而设的多余标题。\n"
        ...
        (
            ...
            "- 在关键事实句末用 [^n] 标注来源编号；只允许使用给定来源清单里的全书固定编号，"
            "不得把本节来源从 [^1] 重新编号，必须逐字复制清单中实际给出的编号，"
            "禁止编造清单外的编号。\n"
            "- 正文禁止出现裸 URL；禁止输出参考文献小节或 [^n]: 脚注定义"
            "（书目由全书汇编统一渲染，不在小节内）。\n"
            "- 易错点仅可使用给定研究笔记切片中带出处的条目；没有带出处的就不要写（诚实省略，禁止编造）。\n"
            "- 对给定易错点必须先写成常见误区再明确驳正，不能只陈述正确结论。\n"
            "- 对区分/辨析/比较任务，必须同时点名比较双方，并用“相比、区别、前者/后者”等明确比较语义。\n"
            "- 仅按用户载荷中的“本节学习闭环硬性配额”生成 [EXn] 带步骤例题、[Qn] 分层自测题、"
            "[An] 答案与评分点；未列出或配额为 0 的标签类型不强制生成。若生成，三类标签各自独立编号"
            "（n 均从 1 起连续递增）、各自独占加粗行，不得合并书写（禁止 **[EX1] [Q1]** 共行）；"
