...
- 当前数据库快照：物理 2,184 题、化学 2,854 题；两科每题均有来源记录，年份均覆盖 2007–2026。答案非空分别为 1,513、2,344，仍全部标记为未核验。
...
- 图形：题面和答案共有 6,230 个本地 SVG 引用（4,884 个唯一文件），另有 8,953 个已登记 SVG 侧车，均已注册、哈希/大小一致且文件存在；远程 `<img src>` 为 0。EOL 2015 新增 9 个 SVG 侧车（题面 5 张、答案 4 张）；EOL 2016–2021 视觉卷新增 95 个 SVG 侧车并复用 1 个，题面 76 张、答案 20 张；EOL 2022–2024 视觉卷新增 126 个 SVG 侧车（题面 86 张、答案 63 张）；另有 3 道化学题的 11 个残留远程图片已转为本地 SVG；2023 PDF 批次新增 81 个整页 SVG 侧车，题目引用 239 次；2023 化学 PDF 新增 29 个整页 SVG 侧车；本轮 2018–2019 正式 PDF/视觉预览/DOCX/OLE 图形批次新增 402 个 SVG 侧车；本轮 2024 新课标理综整卷新增 19 个整页 PDF SVG 侧车，物理/化学题目引用 33 次；本轮 2021 全国甲卷新增 24 个整页/答案页 SVG 侧车；本轮 2025 单科 DOCX 批次新增 473 个 PNG→SVG 侧车，物理/化学题目引用 565 次；本轮 2020 全国卷Ⅱ、Ⅲ理综图像 DOCX 新增 43 个整页 SVG 侧车，物理/化学题目引用 70 次；本轮 2023 年 6 套地方/全国卷嵌套 DOCX 新增 1,108 个媒体 SVG，物理/化学题目引用 1,326 次；本轮 2021 广东 PDF/OLE 新增 39 个媒体 SVG，物理/化学题目引用 44 次；本轮 2021 河北 PDF 新增 24 个整页 SVG，物理/化学题目引用 37 次；本轮 2021 湖南 DOCX/OLE 新增 64 个媒体 SVG，物理/化学题目引用 66 次；本轮 2021 山东 OLE 新增 61 个 PNG→SVG 侧车，物理/化学题目引用 61 次；本轮 2021 浙江物理 OLE 新增 21 个 PNG→SVG 侧车，物理题目引用 21 次；本轮 2017 全国卷II OLE 另将 36 个嵌入 PNG 全部封装为 SVG，其中 20 个题面图形绑定到物理/化学题，16 个来自未导入生物题或答案解析区的媒体保持未绑定；本轮 2024 地方卷 OLE 新增 504 个 PNG→SVG 侧车并绑定 504 次，覆盖 24 份来源文件、409 题（物理 222、化学 187）；本轮 2019 全国卷I OLE 另将 49 个嵌入 PNG 封装为 SVG，其中 24 个题面图形绑定到 14 道物理题，25 个来自生物/化学/答案区的媒体保持未绑定；本轮 2020 全国卷I DOCX 另将 54 个嵌入媒体封装为 SVG，其中 36 个题面图形绑定到 26 道物理/化学题，答案页媒体登记但不挂入题干。SVG 是内嵌原始栅格/原始格式容器，保留来源映射，不宣称完成路径级矢量重绘。
...
- 2019 年全国卷I物理 OLE 补充导入 14 题（题号 14–25、33、34）：从 `.local/imports/xueyeguihua-source/files/2019_2_6.bin` 按理综原卷连续题号拆出物理题，49 个嵌入 PNG 封装为本地 SVG，24 个题面图形按 OLE 对象顺序绑定，其余 25 个生物/化学/答案区媒体保持未绑定。可靠文字片段使用 LaTeX，MathType/OLE 公式与无法可靠转录的图形/表格使用显式 LaTeX 视觉回退并保留原始 SVG；GaokaoHub 仅作目录索引，乱码字段未入库；答案均保留答案页 SVG 提示，`verified=0`。独立审核代理复核三表集合、出处、LaTeX 定界符、24 个 SVG 登记/哈希和数据库完整性，结论通过。脚本为 `.local/imports/xueyeguihua-source/import_2019_national1_physics_ole.py`，报告为 `.local/imports/xueyeguihua-source/import-2019-national1-physics-ole-report.json`。
- 2020 年全国卷I DOCX 补充导入 26 题（物理 14、化学 12）：物理题号 14–25、33、34，化学题号 7–13、26–28、35、36。原卷文字层可读部分按段落拆分；化学第 11、13 题和物理第 19 题保留图形文字层缺失提示，物理第 24 题保留缺失题干开头提示。54 个嵌入媒体全部封装为本地 SVG，36 个按段落媒体顺序绑定到题面，答案页媒体只登记不挂题干；公式和表格按 LaTeX 硬规则处理，其中化学第 26、35 题显式使用 `array`。GaokaoHub 仅作目录索引，乱码字段未入库；答案全部保留答案页 SVG 提示并为 `verified=0`。独立审核代理复核三表集合、出处、LaTeX/array 定界符、36 个题面 SVG 与全部 54 个生成 SVG 的登记/哈希、数据库完整性，结论通过。脚本为 `.local/imports/xueyeguihua-source/import_2020_national1_docx.py`，报告为 `.local/imports/xueyeguihua-source/import-2020-national1-docx-report.json`。
...
- 2019 海南化学 OLE 的 MathType/图形占位符全部保留为 `\(\text{原卷公式见本题SVG}\)` 或 `\(\text{原卷图形见本题SVG}\)`，不从乱码猜公式；第 14 题的时间–浓度表已显式转为 LaTeX `\begin{array}`，原卷页 SVG 仍作为视觉依据。2019 天津卷 OLE 的 MathType/图形占位符沿用同一策略，化学第 2 题离子方程式表已显式转为 LaTeX `\begin{array}`，装置/数据表保留原卷 SVG；2019 全国卷 3 也沿用同一策略；2019 全国卷 1 的扫描 PDF 仍存在物理页重复/缺失，本轮物理题改由独立 OLE 文本批次补入 14 题，但文字和图形仍未核验。
...
- `.local/imports/xueyeguihua-source/import-2019-national1-physics-ole-report.json`
- `.local/imports/xueyeguihua-source/import_2019_national1_physics_ole.py`
- `.local/imports/xueyeguihua-source/import-2020-national1-docx-report.json`
- `.local/imports/xueyeguihua-source/import_2020_national1_docx.py`
...
- 2019 全国卷1物理扫描来源此前存在页重复/缺失；本轮仅以可读取的 OLE 理综文本补入物理 14–25、33–34，文字、答案和 OLE 对象顺序图形均为未核验，仍需完整原卷逐题视觉对照。2020 全国卷1本轮以 DOCX 文字层补入物理 14–25、33–34 与化学 7–13、26–28、35–36；物理 19、化学 11/13 含图形文字层缺失，物理 24 为缺失题干开头的 `partial_text`，均已转 SVG 并待逐题视觉校验。
