...
- 当前数据库快照：物理 1,488 题、化学 2,216 题；两科每题均有来源记录，年份均覆盖 2007–2026。答案非空分别为 817、1,706，仍全部标记为未核验。
...
- 图形：题面和答案共有 3,151 个本地 SVG 引用（2,287 个唯一文件），另有 6,454 个已登记 SVG 侧车，均已注册、哈希/大小一致且文件存在；远程 `<img src>` 为 0。EOL 2015 新增 9 个 SVG 侧车（题面 5 张、答案 4 张）；EOL 2016–2021 视觉卷新增 95 个 SVG 侧车并复用 1 个，题面 76 张、答案 20 张；EOL 2022–2024 视觉卷新增 126 个 SVG 侧车（题面 86 张、答案 63 张）；另有 3 道化学题的 11 个残留远程图片已转为本地 SVG；2023 PDF 批次新增 81 个整页 SVG 侧车，题目引用 239 次；2023 化学 PDF 新增 29 个整页 SVG 侧车；本轮 2018–2019 正式 PDF/视觉预览/DOCX 图形批次新增 381 个 SVG 侧车，其中 2018 江苏化学 DOCX 的 48 个 PNG/JPEG 嵌入图形均已封装为 SVG。SVG 是内嵌原始栅格/原始格式容器，保留来源映射，不宣称完成路径级矢量重绘。
...
- 本轮正式来源补充共 211 题（物理 99、化学 112）：2018 江苏物理 15 题、2018 江苏化学 21 题、2017 全国卷 1/3 理综各 26 题、2019 北京理综 23 题、2019 全国卷 2 理综 26 题、2019 全国卷 1 化学 11 题、2019 全国卷 3 理综 26 题、2019 江苏化学 21 题，以及 2019 江苏物理 16 题；对应原卷/预览/DOCX 嵌入图形共生成 381 个本地 SVG 侧车。2018 江苏物理使用 5 页题面与 6 页答案，2018 江苏化学使用原始 DOCX 的 48 个嵌入图形，2019 全国卷 3 使用完整扫描 PDF，2019 江苏物理使用 10 页题面与 20 页答案/解析预览，DOCX/OLE 文本仅作检索辅助。
...
- 2019 全国卷 3 的 OLE MathType 占位符已统一写为 `\(\text{原卷符号见SVG}\)`，表格写为 `\[\begin{array}{c}\text{原卷表格见 SVG，未猜测表格数值}\end{array}\]`；不使用 GaokaoHub 乱码字段补公式。2019 全国卷 1 的扫描 PDF 存在物理页重复/缺失，因此只导入有完整可读文本与图形映射的化学题，物理部分保留缺口。
- 全库规范化审计已覆盖两科 3,704 行：`rainewhk/gaokao`、GAOKAO-MM 的 `$...$` 已改为 `\(...\)`/`\[...\]`；2023 合订 PDF 与 OLE/DOCX 文本的私有区/控制字形已映射为明确运算符或 `\(\text{原卷符号见SVG}\)`。原始字段保存在 `.local/imports/science-latex-originals.json`，报告为 `.local/imports/science-latex-normalization-report.json`；审计结果为美元符号、私有区字符、HTML 表格、替换字符和残留 EMBED 控制串均为 0。
- 对仍无法从文字层可靠分列的 93 行表格线索，已追加 LaTeX `array` 视觉/来源回退并标记 `table_visual_review_required`；不臆造单元格，报告为 `.local/imports/science-table-latex-fallback-report.json`。
...
- 本轮 2018–2019 正式 PDF/视觉预览/DOCX 图形批次合计 211 道题、381 个 SVG；2018 江苏化学的原始 DOCX 嵌入图形已转 SVG，2019 全国卷 1 物理扫描页不完整，2019 全国卷 3 与 2019 江苏物理的公式/表格仍是 LaTeX 视觉回退，江苏物理书面答案保留答案/解析 SVG，均不能显示为“答案已核验”。
