...
**问题：** `python -m backend.mcp.stdio_server` 报错
...
**原因2：** 导入路径错误
- 检查是否在项目根目录运行
- 确认 `backend/crawler` 和 `backend/database` 模块存在
...
**问题：** `backend/database/models.py` 运行出错
...
1. **组卷网页面结构变化**
   - 打开浏览器访问组卷网
   - 检查页面结构
   - 更新 `backend/crawler/zujuan_crawler.py` 中的CSS选择器
...
```python
# 在 backend/crawler/zujuan_crawler.py 中添加调试
...
```json
{
  "mcpServers": {
    "exam-paper-assistant": {
      "command": "python",
      "args": [
        "完整的绝对路径/study_ai/backend/mcp/stdio_server.py"
      ]
    }
  }
}
...
```python
# 在 backend/mcp/stdio_server.py 的 call_tool 中添加日志
