...
为避免项目根目录堆满数据库、缓存、调试截图、Playwright 浏览器数据等“只和本机有关”的文件，本项目将这些内容统一放到 `exam_paper_assistant/.local/` 下（并在 `.gitignore` 中忽略）。
...
- `exam_paper_assistant/exam_papers.db`
- `exam_paper_assistant/.cache/`
- `exam_paper_assistant/artifacts/`
- `exam_paper_assistant/data/`
- `exam_paper_assistant/.playwright_zujuan_user_data/`
...
```bash
rm -rf exam_paper_assistant/.local
```
