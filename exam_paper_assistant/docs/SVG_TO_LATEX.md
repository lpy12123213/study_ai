...
│   ├── glyph_signatures.json    # 签名映射表（自动生成）
│   ├── signature_analyzer.py    # 签名分析工具
│   └── signature_analyzer.html  # 可视化分析页面
├── crawler/
...
```bash
# 转换单个SVG URL
python svg_to_latex.py -u https://example.com/formula.svg

# 转换本地文件
python svg_to_latex.py -f formula.svg

# 替换HTML中的所有公式
python svg_to_latex.py -H page.html -o output.html

# 使用高级转换（支持分数等复杂结构）
python svg_to_latex.py -u https://example.com/formula.svg --advanced
